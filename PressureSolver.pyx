...
cdef class PressureSolverCollocated:
    def __init__(self):
        pass
    ...
    cpdef update(self,Grid.Grid Gr, ReferenceState.ReferenceState RS,
        ...
        cdef double [:] u3_mean = PM.HorizontalMean(Gr,&PV.values[w_shift])
        #Remove mean u3
        remove_mean_u3_collocated(&Gr.dims,&u3_mean[0],&PV.values[w_shift])
        u3_mean = PM.HorizontalMean(Gr,&PV.values[w_shift])
        ...
        for d in xrange(Gr.dims.dims):
            vel_shift = PV.velocity_directions[d]*Gr.dims.npg
            second_order_divergence_collocated(&Gr.dims, &RS.alpha0[0], &RS.alpha0_half[0],&PV.values[vel_shift],
                 &DV.values[div_shift] ,d)
        ...
        #Apply pressure correction
        second_order_pressure_correction_collocated(&Gr.dims,&DV.values[pres_shift],
                                         &PV.values[u_shift],&PV.values[v_shift],&PV.values[w_shift])
...
cdef void second_order_pressure_correction_collocated(Grid.DimStruct *dims, double *p, double *u, double *v, double *w ):
    ...
    for i in xrange(imin,imax):
        ...
        for j in xrange(jmin,jmax):
            ...
            for k in xrange(kmin,kmax):
                ijk = ishift + jshift + k
                u[ijk] -=  (p[ijk + ip1] - p[ijk-ip1])*dims.dxi[0]*0.5
                v[ijk] -=  (p[ijk + jp1] - p[ijk-jp1])*dims.dxi[1]*0.5
                w[ijk] -=  (p[ijk + kp1] - p[ijk-kp1])*dims.dxi[2]*0.5  #(p[ijk + kp1] - p[ijk])*dims.dxi[2]

    return


cdef void remove_mean_u3_collocated(Grid.DimStruct *dims, double *u3_mean, double *velocity):
...
cdef void second_order_divergence_collocated(Grid.DimStruct *dims, double *alpha0, double *alpha0_half, double *velocity,
                                  double *divergence, Py_ssize_t d):
    ...
    if d != 2:
        for i in xrange(imin,imax):
            ...
            for j in xrange(jmin,jmax):
                ...
                for k in xrange(kmin,kmax):
                     ijk = ishift + jshift + k
                     divergence[ijk] += (( velocity[ijk] + velocity[ijk + sp1])*0.5
                                         - (velocity[ijk] + velocity[ijk+sm1])*0.5)*dxi/alpha0_half[k]
    else:
        for i in xrange(imin,imax):
            ishift = i*istride
            for j in xrange(jmin,jmax):
                ...
                for k in xrange(kmin,kmax):
                     ijk = ishift + jshift + k
                     divergence[ijk] += (( velocity[ijk]/alpha0_half[k] + velocity[ijk + sp1]/alpha0_half[k+1])*0.5
                                         - (velocity[ijk]/alpha0_half[k-1] + velocity[ijk+sm1]/alpha0_half[k-1])*0.5)*dxi
