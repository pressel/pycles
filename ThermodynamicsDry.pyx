...
cdef extern from "thermodynamics_dry.h":
    ...
    void buoyancy_update_collocated(Grid.DimStruct *dims, double *alpha0, double *alpha,double *buoyancy,
                         double *wt)
    void bvf_dry_collocated(Grid.DimStruct* dims,  double* p0, double* T, double* theta, double* bvf)
...
cdef class ThermodynamicsDryCollocated:
    def __init__(self,namelist,LatentHeat LH, ParallelMPI.ParallelMPI Pa):
        self.L_fp = LH.L_fp
        self.Lambda_fp = LH.Lambda_fp
        self.CC = ClausiusClapeyron()
