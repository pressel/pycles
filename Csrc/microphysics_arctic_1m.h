    ...
    for(ssize_t i=imin; i<imax; i++){
        ...
        for(ssize_t j=jmin; j<jmax; j++){
            ...
            for(ssize_t k=kmin; k<kmax; k++){
                ...
                entropy_tendency[ijk]+= qrain[ijk]*(fabs(w_qrain[ijk]) - w[ijk]) * cl * (Twet[ijk+1] - Twet[ijk])* dzi* imetl_half[k]/temperature[ijk];
...
void entropy_source_heating_snow(const struct DimStruct *dims, double* restrict temperature, double* restrict Twet, double* restrict qsnow,
    ...
    for(ssize_t i=imin; i<imax; i++){
        ...
        for(ssize_t j=jmin; j<jmax; j++){
            ...
            for(ssize_t k=kmin; k<kmax; k++){
                const ssize_t ijk = ishift + jshift + k;
                entropy_tendency[ijk]+= qsnow[ijk]*(fabs(w_qsnow[ijk]) - w[ijk]) * ci * (Twet[ijk+1] - Twet[ijk])* dzi* imetl_half[k]/temperature[ijk];
