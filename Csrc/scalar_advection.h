...
void second_order_collocated_a(const struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,const double* restrict velocity, const double* restrict scalar, double* restrict flux, int d){
    ...
    if(d==2){
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    const ssize_t ijk = ishift + jshift + k ;
                    flux[ijk] = interp_2(scalar[ijk],scalar[ijk+sp1]) * interp_2(velocity[ijk]*rho0[k],velocity[ijk+sp1]*rho0[k+1]);
                } // End k loop
            } // End j loop
        } // End i loop
    } // end if
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    const ssize_t ijk = ishift + jshift + k ;
                    flux[ijk] = interp_2(scalar[ijk],scalar[ijk+sp1])* interp_2(velocity[ijk],velocity[ijk+sp1])*rho0_half[k];
                } // End k loop
            } // End j loop
        } // End i loop
    } // end else
    ...
    if(dims->indx_lo[2] == 0){
     ...
     //       for(ssize_t j=jmin; j<jmax; j++){
     //           const ssize_t jshift = j * jstride;
     //           for(ssize_t k=dims->nlg[2] - (2+dims->gw); k<dims->nlg[2]; k++){
     //              //printf("%ld\n",k);
     //               const ssize_t ijk = ishift + jshift + k;
     //               flux[ijk] = 0.0;
     //           }
...
void weno_fifth_order_collocated_a(const struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,const double* restrict velocity, const double* restrict scalar, double* restrict flux, int d){
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    const double  a = (velocity[ijk+sp1]*scalar[ijk + sp1]*rho0_half[k+1] - velocity[ijk]*scalar[ijk]*rho0_half[k])/(velocity[ijk + sp1] - velocity[ijk]);
                    if (a >= 0.0){
                        flux[ijk] = phip;
                        }
                    else{
                    ...
                    //flux[ijk] =  0.5 * ((velocity[ijk]+fabs(velocity[ijk]))*phip + (velocity[ijk]-fabs(velocity[ijk]))*phim);
                } // End k loop
            } // End j loop
        } // End i loop
    } // End if
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    const double phip = interp_weno5(velocity[ijk+sm2]*scalar[ijk + sm2],
                                                     ...
                                                     velocity[ijk+sp2]*scalar[ijk + sp2])*rho0_half[k];
                    ...
                    const double phim = interp_weno5(velocity[ijk+sp3]*scalar[ijk + sp3],
                                                     ...
                                                     velocity[ijk+sp1]*scalar[ijk + sp1],
                                                     velocity[ijk]*scalar[ijk],
                                                     velocity[ijk+sm1]*scalar[ijk + sm1])*rho0_half[k];


                    const double  a = (velocity[ijk+sp1]*scalar[ijk + sp1] - velocity[ijk]*scalar[ijk])/(velocity[ijk + sp1] - velocity[ijk]);
                    if (a >= 0.0){
                        flux[ijk] = phip;
                        }
                    else{
                    ...
                    // flux[ijk] =  0.5 * ((velocity[ijk]+fabs(velocity[ijk]))/velocity[ijk]*phip + (velocity[ijk]-fabs(velocity[ijk]))*phim);
                } // End k loop
            } // End j loop
        } // End i loop
    } // End else
...
void compute_advective_fluxes_collocated_a(struct DimStruct *dims, double* restrict rho0, double* rho0_half ,double* restrict velocity, double* restrict scalar,
                                double* restrict flux, int d, int scheme){
    switch(scheme){
        case 1:
            upwind_first_a(dims, rho0, rho0_half, velocity, scalar, flux, d);
            break;
        case 2:
            second_order_collocated_a(dims, rho0, rho0_half, velocity, scalar, flux, d);
            break;
        case 3:
            weno_third_order_a(dims, rho0, rho0_half, velocity, scalar, flux, d);
            break;
        case 4:
            ...
            break;
        case 5:
            weno_fifth_order_collocated_a(dims, rho0, rho0_half, velocity, scalar, flux, d);
            break;
        case 6:
            sixth_order_a(dims, rho0, rho0_half, velocity, scalar, flux, d);
            break;
