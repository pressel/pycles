...
// Need this for fmin and fmax
#include <math.h>

// The second_order_a scheme has been collocated and the weno_fifth_order_a scheme as well.
// The factor alpha in the Rusanov scheme might not be off by a factor rho0.
    ...
    if(d==2){
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    flux[ijk] = interp_2(scalar[ijk] * velocity[ijk] * rho0[k],scalar[ijk+sp1] * velocity[ijk+sp1] * rho0[k+sp1]);
                } // End k loop
            } // End j loop
        } // End i loop
    } // end if
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    const ssize_t ijk = ishift + jshift + k ;
                    flux[ijk] = interp_2(scalar[ijk] * velocity[ijk] * rho0[k],scalar[ijk+sp1] * velocity[ijk+sp1] * rho0[k]);
                } // End k loop
            } // End j loop
        } // End i loop
    } // end else
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    // Employing a Rusanov/local Lax-Friedrichs flux
                    flux[ijk] =  0.5 * ((scalarp * vp * rho0p + scalarm * vm * rho0m) - fmax(fabs(vp),fabs(vm)) * (scalarp-scalarm));
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    // Employing a Rusanov/local Lax-Friedrichs flux
                    flux[ijk] =  0.5 * ((scalarp * vp * rho0_half[k] + scalarm * vm * rho0_half[k]) - fmax(fabs(vp),fabs(vm)) * (scalarp - scalarm));
        ...
        for(ssize_t i=imin;i<imax;i++){
            ...
            for(ssize_t j=jmin;j<jmax;j++){
                ...
                for(ssize_t k=kmin;k<kmax;k++){
                    ...
                    // Emplyoing a Rusanov/local Lax-Friedrichs scheme
                    flux[ijk] =  0.5 * ((velocity[ijk]+fabs(velocity[ijk]))*phip + (velocity[ijk]-fabs(velocity[ijk]))*phim)*rho0[k];
