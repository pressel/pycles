...
cdef class RadiationDyCOMS_RF01:
    ...
    cpdef update(self, Grid.Grid Gr, ReferenceState.ReferenceState Ref,
        ...
        cdef:
            ...
            double [:] rad_flux = np.zeros((Gr.dims.npg,),dtype=np.double,order='c')
        ...
        with nogil:
            for pi in xrange(self.z_pencil.n_local_pencils):
                ...
                #Now compute the third term on RHS of Stevens et al 2005 (equation 3)
                f_rad[pi,0] = rhoi * cpd * self.divergence * self.alpha_z * ( cbrt(pow(z[gw-1] - zi,4))/4.0
                                                                                      + zi*cbrt(z[gw-1] - zi))
                for k in xrange(Gr.dims.n[2]+1):
                     f_rad[pi,k+1] = rhoi * cpd * self.divergence * self.alpha_z * ( cbrt(pow(z[gw+k] - zi,4))/4.0
                                                                                     + zi*cbrt(z[gw+k] - zi))
                #Compute the second term on RHS of Stevens et al. 2005 (equation 3)
                q_1 = 0.0
                f_rad[pi,0] += self.f1 * exp(-q_1)
                for k in xrange(1,Gr.dims.n[2]+1):
                    q_1 += self.kappa * rho_half[gw+k] * ql_pencils[pi,k] * dz
                    f_rad[pi,k] += self.f1 * exp(-q_1)

                #Compute the first term on RHS of Stevens et al. 2005 (equation 3)
                q_0 = 0.0
                f_rad[pi,Gr.dims.n[2]] += self.f0 * exp(-q_0)
                for k in xrange(Gr.dims.n[2]-1,-1,-1):
                    q_0 += self.kappa * rho_half[gw+k] * ql_pencils[pi,k]* dz
                    f_rad[pi,k] +=  self.f0 * exp(-q_0)


        #Now transpose the flux pencils
        #self.z_pencil.reverse_double(&Gr.dims,Pa,f_rad,&rad_flux[0])
        import pylab as plt
        plt.figure(1)
        plt.plot(f_rad[0,:])
        plt.show()
